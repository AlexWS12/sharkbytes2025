...
print("Testing angles around the problem zones (80deg and 120deg)...")
...
for angle in test_angles:
    ...
    response = input(f"Angle {angle:3d}deg - Jittery? (y/n/quit): ").lower().strip()
    
    if response == 'quit' or response == 'q':
        break
    elif response == 'y' or response == 'yes':
        jitter_angles.append(angle)
        print(f"  [FAIL] Jitter detected at {angle}deg")
    else:
        print(f"  [OK] Smooth at {angle}deg")
...
# Return to center
print("Returning to center (90deg)...")
